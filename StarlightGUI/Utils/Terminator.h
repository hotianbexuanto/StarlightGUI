...
static P_EndTask EndTaskAddr{ nullptr };
static P_WinStationOpenServerW fpWinStationOpenServerW = NULL;
static P_WinStationTerminateProcess fpWinStationTerminateProcess = NULL;
...
namespace winrt::StarlightGUI::implementation {
	...
	static DWORD GetProcessPIDByName(std::string procName) {
		DWORD pid = 0;
	...
	static int GetPID(std::string input) {
	...
	static HANDLE GetProcessByPID(int pid) {
		// Open process
	...
	static DWORD WINAPI RemoteThread_ExitProcess(LPVOID lpParam) {
	...
	static BOOL WinstaKillProcess(PWSTR hServer, DWORD dwPid, ULONG exitCode) {
	...
	static bool PatchThread64(DWORD pid) {
#ifdef _WIN64
	...
	// [1] TerminateProcess
	static bool TerminateProcessNormally(int pid) {
		HANDLE hProc = GetProcessByPID(pid);
	...
	// [2] PostMessage
	static BOOL CALLBACK TerminateProcessByWindow(HWND hwnd, LPARAM lparam) {
	...
	static bool TerminateProcessNT(int pid) {
		HANDLE hProc = GetProcessByPID(pid);
	...
	// [4] TerminateThread
	static bool TerminateProcessByThread(int pid) {
	...
	static bool TerminateProcessByJob(int pid) {
		HANDLE hProc = GetProcessByPID(pid);
	...
	// [6] EndTask
	static BOOL CALLBACK TerminateProcessForce(HWND hwnd, LPARAM lparam) {
		DWORD* procPid = (DWORD*)lparam;
		DWORD windowPid;
	...
	static bool TerminateProcessByInjection(int pid) {
		HANDLE hProc = GetProcessByPID(pid);
	...
	static bool TerminateProcessByDebugger(int pid) {
	...
	static bool TerminateProcessWinsta(int pid) {
	...
	static bool TerminateProcessByPatch(int pid) {
		HANDLE hSnapshot = CreateToolhelp32Snapshot(TH32CS_SNAPTHREAD, (DWORD)pid);
		HANDLE hProc = GetProcessByPID(pid);
	...
	static bool TerminateProcessByMemoryZero(int pid) {
		HANDLE hProc = GetProcessByPID(pid);
	...
	static bool DoTerminateProcess(int method, int pid, XamlRoot& xamlRoot, Panel panel) {
		t_root = xamlRoot;
		t_parent = panel;
		switch (method) {
		case 1:
	...
	static bool TerminateProcessByKernel(int pid, std::wstring& content) {
